...
import { MapPin, Navigation, History, Star, Home, Briefcase } from 'lucide-react';
            ...
            {suggestions.length > 0 ? (
                <div className="space-y-1">
                    ...
                    {suggestions.map((elem, idx) => (
                        <motion.div
                            ...
                            className='flex gap-4 p-4 hover:bg-slate-50 cursor-pointer transition-all items-center justify-start rounded-[1.5rem] group border border-transparent hover:border-slate-100'
                        >
                            <div className='bg-slate-100 h-12 w-12 flex-shrink-0 flex items-center justify-center rounded-2xl text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-sm'>
                                <MapPin className="w-5 h-5" />
                            </div>
                            <div className="flex-1 overflow-hidden">
                                <h4 className='font-bold text-sm text-slate-700 truncate group-hover:text-slate-900'>{elem}</h4>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Precise Match</p>
                            </div>
                        </motion.div>
                    ))}
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <div key={idx} className="p-4 bg-slate-50 border border-slate-100 rounded-3xl hover:bg-white hover:shadow-xl hover:shadow-indigo-500/5 transition-all cursor-pointer group">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-4 p-4 rounded-3xl bg-indigo-50/50 border border-indigo-100 text-indigo-700 cursor-pointer hover:bg-indigo-100 transition-all group">
